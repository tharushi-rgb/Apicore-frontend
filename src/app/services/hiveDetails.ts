...
export const feedingsService = {
  ...
  async update(id: number, p: Partial<Feeding>) {
    ...
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Feeding', event: 'updated', details: 'Feeding record was updated.' });
    return data as Feeding;
  },
  async delete(id: number) {
    const { error } = await supabase.from('feedings').delete().eq('id', id);
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Feeding', event: 'deleted', details: `Feeding #${id} was removed.` });
  },
};
...
export const componentsService = {
  ...
  async update(id: number, p: Partial<HiveComponent>) {
    ...
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Component', event: 'updated', details: 'Hive component was updated.' });
    return data as HiveComponent;
  },
  async delete(id: number) {
    const { error } = await supabase.from('hive_components').delete().eq('id', id);
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Component', event: 'deleted', details: `Hive component #${id} was removed.` });
  },
};
...
export const queensService = {
  ...
  async update(id: number, p: Partial<Queen>) {
    ...
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Queen', event: 'updated', details: 'Queen record was updated.' });
    return data as Queen;
  },
  async delete(id: number) {
    const { error } = await supabase.from('queens').delete().eq('id', id);
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Queen', event: 'deleted', details: `Queen #${id} was removed.` });
  },
};
...
export const treatmentsService = {
  ...
  async update(id: number, p: Partial<Treatment>) {
    ...
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Treatment', event: 'updated', details: 'Treatment record was updated.' });
    return data as Treatment;
  },
  async delete(id: number) {
    const { error } = await supabase.from('treatments').delete().eq('id', id);
    if (error) throw new Error(error.message);
    notificationsService.createActionNotification({ entity: 'Treatment', event: 'deleted', details: `Treatment #${id} was removed.` });
  },
};
