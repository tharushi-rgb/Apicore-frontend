...
export const dashboardService = {
  async get(): Promise<DashboardData> {
    ...
    const [
      ...
      supabase.from('hives').select('*, apiaries(name)').eq('user_id', userId),
    ...
    const apiaries: Apiary[] = (apiaryRows ?? []).map((a: any) => ({
      id: a.id, name: a.name, district: a.district, status: a.status, hive_count: 0,
    }));
    ...
    const hives: Hive[] = (hiveRows ?? []).map((h: any) => ({
      id: h.id, name: h.name, hive_type: h.hive_type, status: h.status,
