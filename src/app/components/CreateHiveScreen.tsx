...
export function CreateHiveScreen({ selectedLanguage, onLanguageChange, onNavigate, onClose, contextApiary, initialHive, onLogout }: Props) {
  ...
  const parsedInitial = useMemo(() => parseHiveNotes(initialHive?.notes), [initialHive?.notes]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isBoxLike && form.entrance_positions.length === 0) {
    ...
    try {
      ...
      const sharedData: any = {
        ...
        apiary_id: form.apiary_selection === 'standalone' ? null : parseInt(form.apiary_selection, 10),
        location_type: form.apiary_selection === 'standalone' ? 'standalone' : 'apiary-linked',
        status: form.status,
    ...
    <div className="h-[100dvh] bg-gradient-to-b from-amber-50 via-emerald-50 to-amber-100 flex flex-col overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="px-4 py-5 space-y-4 pb-24">
        ...
        <div className="bg-white rounded-2xl p-4 border border-stone-200 space-y-3">
          ...
          <div>
            ...
            <select value={form.apiary_selection} onChange={(e) => updateField('apiary_selection', e.target.value)} className={inputCls}>
              <option value="standalone">Standalone (None)</option>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <select value={form.status} onChange={(e) => updateField('status', e.target.value)} className={inputCls}>
                ...
                <option value="given_for_free">Given for free</option>
                <option value="empty">Empty</option>
                <option value="absconded">Absconded (Heerayama)</option>
        ...
        <div className="bg-white rounded-2xl p-4 border border-stone-200 space-y-3">
          ...
          {isBoxLike && (
            <div className="space-y-3 rounded-xl bg-amber-50/50 border border-amber-100 p-3">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div>
                  <label className={sLabelCls}>Bottom (Optional)</label>
                  <input value={form.bottom_text} onChange={(e) => updateField('bottom_text', e.target.value)} placeholder="e.g., Anti-ant bottom" className={inputCls} />
                </div>
              </div>
              ...
              <div>
                <label className={sLabelCls}>Number of Entrances (Optional)</label>
                <input type="number" min="0" value={form.num_entrances} onChange={(e) => updateField('num_entrances', e.target.value)} className={inputCls} />
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              <label className={labelCls}>Entrance Guard (Optional)</label>
              <div className="flex gap-2">
                {['yes', 'no'].map((value) => (
                  <button key={value} type="button" onClick={() => updateField('entrance_guard', value)} className={`flex-1 py-2 rounded-xl text-sm border ${form.entrance_guard === value ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-stone-700 border-stone-200'}`}>
                    {value === 'yes' ? 'Yes' : 'No'}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className={labelCls}>Ant Protection *</label>
              <div className="flex gap-2">
                {['yes', 'no'].map((value) => (
                  <button key={value} type="button" onClick={() => updateField('ant_protection', value)} className={`flex-1 py-2 rounded-xl text-sm border ${form.ant_protection === value ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-stone-700 border-stone-200'}`}>
                    {value === 'yes' ? 'Yes' : 'No'}
                  </button>
                ))}
              </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-stone-200 space-y-3">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div>
              <label className={labelCls}>Date of Queen Acceptance / Hatching</label>
        ...
        <div className="bg-white rounded-2xl p-4 border border-stone-200 space-y-3">
          ...
          <div className="space-y-2">
            ...
            <input type="file" accept="image/*" multiple onChange={handleImageUpload} className="w-full text-sm text-stone-600" />
