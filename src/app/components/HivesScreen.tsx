      ...
      <div className="px-4 py-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-xl p-3 shadow-sm text-center">
            ...
            <p className="text-xl font-bold text-stone-800">{hives.length}</p>
          </div>
          <div className="bg-white rounded-xl p-3 shadow-sm text-center">
            <p className="text-stone-600 text-xs">Active</p>
            <p className="text-xl font-bold text-emerald-600">{hives.filter(h => h.status === 'active').length}</p>
          </div>
          <div className="bg-white rounded-xl p-3 shadow-sm text-center">
            <p className="text-stone-600 text-xs">Queenless</p>
            <p className="text-xl font-bold text-red-600">{hives.filter(h => h.status === 'queenless').length}</p>
          </div>
          <div className="bg-white rounded-xl p-3 shadow-sm text-center">
            <p className="text-stone-600 text-xs">Starred</p>
            <p className="text-xl font-bold text-amber-600">{hives.filter(h => h.is_starred).length}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto hide-scrollbar">
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="bg-white border border-stone-200 rounded-xl px-3 py-2 text-sm focus:border-amber-500 focus:outline-none">
          ...
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-white border border-stone-200 rounded-xl px-3 py-2 text-sm focus:border-amber-500 focus:outline-none">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-stone-500">No hives found</p>
          </div>
        ) : (
