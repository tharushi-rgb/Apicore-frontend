      ...
      <div className="flex-1 flex flex-col px-[6%] pt-[0.75rem] pb-[2rem]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center space-y-4 max-w-md mx-auto w-full">
          <button onClick={() => setSelectedRole('beekeeper')}
            ...
            <h2 className="text-xl font-bold text-stone-800 mb-2">Beekeeper (Admin)</h2>
            <p className="text-stone-600 text-center text-sm">Manage apiaries, hives, finances, helpers, and client services.</p>
          </button>
          <button onClick={() => setSelectedRole('helper')}
            className={`w-full bg-white rounded-2xl p-6 transition-all min-h-[160px] flex flex-col items-center justify-center ${selectedRole === 'helper' ? 'border-4 border-emerald-500 shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'border-2 border-stone-200 hover:border-emerald-300 hover:shadow-lg'}`}>
            <div className="flex items-center justify-center mb-4">
              <ClipboardCheck className="w-16 h-16 text-emerald-600 stroke-[1.5]" />
            </div>
            <h2 className="text-xl font-bold text-stone-800 mb-2">Helper / Staff</h2>
            <p className="text-stone-600 text-center text-sm">Perform inspections and tasks assigned by a Beekeeper.</p>
          </button>
        </div>
      </div>
