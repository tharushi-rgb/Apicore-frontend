...
// ...existing code...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-white rounded-xl p-3 shadow-sm text-center"><p className="text-stone-600 text-xs">Empty</p><p className="text-2xl font-bold text-amber-600">{apiaries.filter(a=>a.status==='empty').length}</p></div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12"><MapPin className="w-12 h-12 text-stone-300 mx-auto mb-3" /><p className="text-stone-500">No apiaries found</p></div>
        ) : (
              ...
              <div key={a.id} className="bg-white rounded-xl p-4 shadow-sm">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-stone-600 flex items-center gap-1 mt-1"><MapPin className="w-3 h-3" />{a.district}{a.area ? `, ${a.area}` : ''}</p>
                  </div>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${a.status==='active'?'bg-emerald-100 text-emerald-700':a.status==='expired'?'bg-red-100 text-red-700':'bg-stone-100 text-stone-600'}`}>{a.status}</span>
                </div>
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex gap-2">
                    {onViewApiary && <button onClick={() => onViewApiary(a.id)} className="p-2 bg-blue-50 rounded-lg hover:bg-blue-100"><Eye className="w-4 h-4 text-blue-600" /></button>}
                    {onAddHive && <button onClick={() => onAddHive(a)} className="p-2 bg-amber-50 rounded-lg hover:bg-amber-100"><Plus className="w-4 h-4 text-amber-600" /></button>}
                  </div>
                </div>
              </div>
            ))}
