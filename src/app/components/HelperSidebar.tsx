...
function NavItem({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-3 transition-colors ${active ? 'bg-emerald-100 text-emerald-700 border-l-4 border-emerald-500' : 'text-stone-700 hover:bg-stone-100'}`}>
