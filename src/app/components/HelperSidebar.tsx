import { Home, Hexagon as HiveIcon, MapPin, User, LogOut } from 'lucide-react';

export type HelperNavTab = 'dashboard' | 'myHives' | 'myApiaries' | 'profile';

interface HelperSidebarProps {
  isOpen: boolean;
  activeTab: HelperNavTab;
  onNavigate: (tab: HelperNavTab) => void;
  onClose: () => void;
  onLogout?: () => void;
}

interface NavItemProps {
  icon: React.ReactNode;
  label: string;
  active: boolean;
  onClick: () => void;
}

function NavItem({ icon, label, active, onClick }: NavItemProps) {
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-3 px-4 py-3 transition-colors ${
        active
          ? 'bg-emerald-100 text-emerald-700 border-l-4 border-emerald-500'
          : 'text-stone-700 hover:bg-stone-100'
      }`}
    >
      <div className="w-5 h-5">{icon}</div>
      <span className="font-medium">{label}</span>
    </button>
  );
}

export function HelperSidebar({ isOpen, activeTab, onNavigate, onClose, onLogout }: HelperSidebarProps) {
  const handleNavClick = (tab: HelperNavTab) => {
    onNavigate(tab);
    onClose();
  };

  return (
    <>
      {/* Overlay */}
      {isOpen && (
        <div
          className="absolute inset-0 bg-black/50 z-40"
          onClick={onClose}
          aria-hidden="true"
        />
      )}

      {/* Sidebar */}
      <div
        className={`absolute top-0 left-0 bottom-0 bg-white shadow-2xl z-50 transition-transform duration-300 ease-in-out w-72 flex flex-col ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        {/* Logo/Header */}
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 px-6 py-6 flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="bg-white/20 p-2 rounded-lg">
              <HiveIcon className="w-8 h-8 text-white" />
            </div>
            <div>
              <h2 className="font-bold text-white text-xl">ApiCore</h2>
              <p className="text-emerald-100 text-sm">Helper Dashboard</p>
            </div>
          </div>
        </div>

        {/* Navigation Items - Scrollable */}
        <nav className="py-4 flex-1 overflow-y-auto">
          <NavItem
            icon={<Home className="w-5 h-5" />}
            label="Dashboard"
            active={activeTab === 'dashboard'}
            onClick={() => handleNavClick('dashboard')}
          />
          <NavItem
            icon={<HiveIcon className="w-5 h-5" />}
            label="My Hives"
            active={activeTab === 'myHives'}
            onClick={() => handleNavClick('myHives')}
          />
          <NavItem
            icon={<MapPin className="w-5 h-5" />}
            label="My Apiaries"
            active={activeTab === 'myApiaries'}
            onClick={() => handleNavClick('myApiaries')}
          />
          <NavItem
            icon={<User className="w-5 h-5" />}
            label="Profile"
            active={activeTab === 'profile'}
            onClick={() => handleNavClick('profile')}
          />
        </nav>

        {/* Footer - Fixed at bottom */}
        <div className="p-4 border-t border-stone-200 bg-stone-50 flex-shrink-0">
          {onLogout && (
            <button
              onClick={() => {
                onLogout();
                onClose();
              }}
              className="w-full flex items-center justify-center gap-2 py-3 mb-3 rounded-lg bg-red-50 text-red-700 font-semibold hover:bg-red-100 transition-colors"
            >
              <LogOut className="w-5 h-5" />
              <span>Log out</span>
            </button>
          )}
          <p className="text-xs text-stone-600 text-center">ApiCore v1.0.0 — Helper</p>
        </div>
      </div>
    </>
  );
}
